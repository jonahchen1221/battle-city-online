...
- 普通战场 320×240，位于 (16, 8)；右侧 32px 为 HUD 栏（x = FIELD_X + FIELD_WIDTH）。护送关世界为 640×720，仍通过 320×240 视口渲染。
- 20×15 大格（16px，坦克尺寸）= 40×30 子格（8px，砖块破坏单位）。
- 鹰巢在底部正中（子格列 19-20、行 28-29）；玩家出生列 6/14/24/32、敌方出生点在上半场随机（可通行且不压车，见 enemy.ts pickSpawnSpot）。
...
- `npm start` — 单端口托管 dist（HTTP 静态）+ WS（需先 `npm run build`）
- `npm run lan` — 局域网联机一键启动（构建 + 启动，终端打印局域网访问地址，朋友同 WiFi 直接打开）
