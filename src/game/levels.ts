...
// 十二关地形。全部 40×30 子格（20×15 大格），对称设计，逐关地形更丰富。
// 普通关（1–5、7–11）共同约束：鹰巢（列 19-20 行 28-29）+ 经典 1 子格厚砖墙护盾
//（列 18-21 行 27 / 列 18,21 行 28-29）；顶部两行（0-1）全空供敌军出生；
// 底部玩家出生列（6/14/24/32）在行 28-29 留空。
// Boss 关（6、12）的差异见文件末尾 STAGE_BOSS_* 的注释：无鹰巢、上半场正中留 Boss 空域。
// 注：STAGE_6_ROWS … STAGE_10_ROWS 是历史命名，插入两张 Boss 图后它们对应第 7–11 关。
...
//   2. 上半场正中留出 Boss 的停驻空域（子格 cols 16–23 × rows 4–13 全空，Boss 车体占 17–22 / 6–11）；
...
// Boss 关 A（第 6 关）：环形斗兽场 —— 上半场只在两翼立柱，中央整片空场让 Boss 一览无余；
...
// Boss 关 B（第 12 关，最终战）：残破的决战场 —— 掩体更少、更薄、缺口更大，
